...
class PayPalPaymentProvider extends base_provider_1.PayPalProviderBase {
    ...
    async authorizePayment(input) {
        ...
        if (orderId) {
            try {
                ...
                if (data.captured_at) {
                    return {
                        status: "captured",
                        data: { ...(input.data || {}), captured_at: data.captured_at },
                    };
                }
        ...
        throw new utils_1.MedusaError(utils_1.MedusaError.Types.NOT_ALLOWED, "No approved PayPal payment was found for this session. The buyer must approve the payment with PayPal before the cart can be completed.");
    }
    ...
    async capturePayment(input) {
        ...
        try {
            ...
            throw error instanceof utils_1.MedusaError
                ? error
                : new utils_1.MedusaError(utils_1.MedusaError.Types.INVALID_DATA, error?.message || "PayPal capture failed.");
        }
    }
    async refundPayment(input) {
        ...
        const refundAmount = (0, amounts_1.toAmountNumber)(input.amount);
        ...
        const requestId = this.getIdempotencyKey(input, `refund-${captureId}-${refundAmount}`);
        const currencyOverride = await this.resolveCurrencyOverride();
        ...
        const refundValue = refundAmount > 0
            ? (0, amounts_1.formatAmountForPayPal)(refundAmount, currencyCode)
            : null;
        let debugId = null;
        try {
            const { accessToken, base } = await this.getPayPalAccessToken();
            ...
            const ppResp = await (0, paypal_fetch_1.paypalFetchWithRetry)(`${base}/v2/payments/captures/${encodeURIComponent(captureId)}/refund`, {
                method: "POST",
                headers: {
                    Authorization: `Bearer ${accessToken}`,
                    "Content-Type": "application/json",
            ...
            if ((0, status_utils_1.isRefundFailureStatus)(refundStatus)) {
                throw new Error(`PayPal refund did not succeed (status=${refundStatus}). The refund was not issued.` +
            ...
            throw error instanceof utils_1.MedusaError
                ? error
                : new utils_1.MedusaError(utils_1.MedusaError.Types.INVALID_DATA, error?.message || "PayPal refund failed.");
        }
    }
