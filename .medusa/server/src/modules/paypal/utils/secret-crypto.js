...
exports.isSecretEncryptionEnabled = isSecretEncryptionEnabled;
 ...
 */
function assertOrWarnNoEncryptionKey() {
    const strict = String(process.env.PAYPAL_ENCRYPTION_STRICT || "").toLowerCase();
    if (strict === "true" || strict === "1") {
        throw new Error("PAYPAL_ENCRYPTION_STRICT is enabled but PAYPAL_ENCRYPTION_KEY is not set. " +
            "Set PAYPAL_ENCRYPTION_KEY to encrypt PayPal secrets at rest, or disable strict mode.");
    }
    if (!warnedNoKey) {
        warnedNoKey = true;
        const inProd = process.env.NODE_ENV === "production";
        console.warn(`[PayPal]${inProd ? " WARNING (production):" : ""} PAYPAL_ENCRYPTION_KEY is not set — ` +
