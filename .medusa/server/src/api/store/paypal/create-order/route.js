...
async function attachPayPalOrderToSession(req, cartId, orderId) {
    const session = await (0, payment_session_1.findPayPalSessionForCart)(cartId, req.scope);
    if (!session) {
        return;
    }
...
async function getExistingPayPalOrderId(req, cartId) {
    const session = await (0, payment_session_1.findPayPalSessionForCart)(cartId, req.scope);
    return (0, payment_session_1.getStoredPayPalOrderId)(session?.session_data);
}
...
async function POST(req, res) {
    ...
    try {
        ...
        const isCardPayment = !!body.is_card_payment;
        ...
        const advancedCardSettings = (settingsData.advanced_card_payments || {});
        const threeDsRaw = typeof advancedCardSettings.threeDS === "string"
            ? advancedCardSettings.threeDS
            : "when_required";
        const threeDsMethod = isCardPayment
            ? (threeDsRaw === "always"
                ? "SCA_ALWAYS"
                : "SCA_WHEN_REQUIRED")
            : null;
        ...
        const exponent = (0, amounts_1.getCurrencyExponent)(currency);
        ...
        if (existingOrderId) {
            try {
                const checkBase = (0, paypal_auth_1.getPayPalApiBase)(creds.environment);
                const checkToken = await paypal.getAppAccessToken();
                const checkResp = await (0, paypal_fetch_1.paypalFetch)(`${checkBase}/v2/checkout/orders/${encodeURIComponent(existingOrderId)}`, {
                    method: "GET",
                    headers: {
                ...
                if (checkResp.ok) {
                    ...
                    const reusableStatus = ["CREATED", "APPROVED", "PAYER_ACTION_REQUIRED"].includes(existingStatus);
                    const amountMatches = String(existingAmount?.value || "") === value &&
                    ...
                    logger.info(`[paypal] create-order: stored order ${existingOrderId} is stale (status=${existingStatus}, amount=${existingAmount?.value} ${existingAmount?.currency_code} vs cart ${value} ${currency}) — creating a fresh order (request_id=${requestId})`);
                }
        ...
        const statementName = typeof additionalSettings.creditCardStatementName === "string"
        ...
        const purchaseItemsRaw = sendItemDetails
            ? lineItems
                ...
                const lineSubtotalMajor = Number(item?.subtotal ?? (Number(item?.unit_price || 0) * quantity));
                const unitAmount = quantity > 0 ? parseFloat((lineSubtotalMajor / quantity).toFixed(exponent)) : 0;
                ...
                .filter(Boolean)
            : [];
        const roundedItemSum = purchaseItemsRaw.reduce((sum, item) => sum + item.unitAmount * item.quantity, 0);
        const roundedItemSumFixed = parseFloat(roundedItemSum.toFixed(exponent));
        const finalPurchaseItems = purchaseItemsRaw.map((item) => item.paypalItem);
        ...
        const itemsTotalMajor = sendItems
            ? parseFloat(finalPurchaseItems
                .reduce((sum, it) => sum + Number(it.unit_amount.value) * Number(it.quantity), 0)
                .toFixed(exponent))
            : 0;
        ...
        if (sendItems) {
            ...
            const breakdownSum = parseFloat((itemsTotalMajor + shippingMajor + taxMajor - discountValue).toFixed(exponent));
            if (Math.abs(breakdownSum - totalMajor) > 0.000001) {
                const gap = parseFloat((totalMajor - breakdownSum).toFixed(exponent));
                if (gap > 0) {
                    breakdown.tax_total = {
                        currency_code: currency,
                        value: parseFloat(((Number(breakdown.tax_total?.value || 0) + gap).toFixed(exponent))).toFixed(exponent),
                    };
                }
                else {
        ...
        logger.debug(`[paypal] create-order line items (request_id=${requestId}): send_item_details=${sendItemDetails}, cart_items=${lineItems.length}, sent_items=${finalPurchaseItems.length}, items_total=${itemsTotalMajor}, subtotal=${subtotalMajor}, total=${totalMajor}`);
        ...
        const body = (req.body || {});
        logger.error(`[paypal] create-order failed (request_id=${requestId}, cart_id=${body.cart_id ?? "n/a"}, debug_id=${debugId ?? "n/a"}): ${e?.message ?? String(e)}`, e instanceof Error ? e : undefined);
        try {
        ...
        const isCurrencyError = rawMessage.includes("PayPal does not support currency") ||
