...
async function paypalWebhookRetry(container) {
    ...
    let pg = null;
    try {
        pg = container.resolve(utils_1.ContainerRegistrationKeys.PG_CONNECTION);
    }
    catch {
        pg = null;
    }
    ...
    try {
        await pg.transaction(async (trx) => {
            const res = await trx.raw("SELECT pg_try_advisory_xact_lock(?) AS locked", [RETRY_ADVISORY_LOCK_KEY]);
...
async function runRetrySweep(container) {
    const paypal = container.resolve("paypal_onboarding");
    const now = Date.now();
    // 1. Failed events whose backoff has elapsed.
    const failed = await paypal.listPayPalWebhookEvents({ status: "failed" }, { take: RETRY_BATCH_SIZE, order: { next_retry_at: "ASC" } });
    const dueFailed = (failed || []).filter((event) => {
    ...
    const processing = await paypal.listPayPalWebhookEvents({ status: "processing" }, { take: RETRY_BATCH_SIZE, order: { updated_at: "ASC" } });
    const staleProcessing = (processing || []).filter((event) => {
    ...
    if (staleProcessing.length) {
        console.warn(`[PayPal] webhook-retry: recovering ${staleProcessing.length} stale "processing" event(s)`);
    }
    const candidates = [...dueFailed, ...staleProcessing];
    if (!candidates.length)
        return;
    console.info(`[PayPal] webhook-retry: evaluating ${candidates.length} event(s) (${dueFailed.length} failed, ${staleProcessing.length} stale)`);
    for (const event of candidates) {
        await attemptRetry(paypal, container, event);
    }
}
async function attemptRetry(paypal, container, event) {
