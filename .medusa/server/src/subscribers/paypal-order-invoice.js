...
async function paypalOrderInvoiceHandler({ event, container, }) {
    ...
    try {
        ...
        try {
            const statusResp = await (0, paypal_fetch_1.paypalFetch)(`${base}/v2/checkout/orders/${encodeURIComponent(paypalOrderId)}`, { headers: { Authorization: `Bearer ${accessToken}` } });
            if (statusResp.ok) {
        ...
        const patchResp = await (0, paypal_fetch_1.paypalFetch)(`${base}/v2/checkout/orders/${encodeURIComponent(paypalOrderId)}`, {
            method: "PATCH",
            headers: {
